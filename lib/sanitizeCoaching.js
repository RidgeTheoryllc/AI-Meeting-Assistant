...
function buildSayFallback(options = {}) {
  ...
  if (intent.askingPrice && intent.clientStatedBudget) {
    const budget = intent.clientBudgetText || 'your stated budget'
    return `For ${budget}, phase one covers a refreshed marketing site plus online booking with appointment scheduling. Insurance eligibility can ship as a lightweight API hook in phase one or move to phase two if QA time is tight.`
  }
...
export function stripInventedPricing(text, { allowPricing = false, documentedPrices = [], clientStatedPrices = [] } = {}) {
  ...
  return text.replace(INVENTED_PRICE_PATTERN, (match) => {
    ...
    if (!allowPricing && numeric > 0 && numeric < 500 && !/k\b/i.test(lower)) {
...
export function sanitizeCoachingResponse(text, options = {}) {
  ...
  if (sayMatch) {
    ...
    if (!sanitizedSay.trim() && intent.askingPrice && intent.clientStatedBudget) {
      const budget = intent.clientBudgetText || 'eight thousand dollars'
      sanitizedSay = `For ${budget}, phase one covers a refreshed marketing site plus online booking with appointment scheduling. Insurance eligibility can ship as a lightweight API hook in phase one or move to phase two if QA time is tight.`
    }
