...
- Spreadsheet rows are PAST CLIENTS we built for. They are NOT ${prospect}'s systems, vendors, or stated needs.
- Portfolio names (${names.slice(0, 12).join(', ')}${names.length > 12 ? ', ...' : ''}) may ONLY be cited as "we built X for [name]" — never as tools ${prospect} uses or wants.
- NEVER say "you're looking for", "I understand you need", or "integrates with [portfolio name]" unless the CLIENT or PRIOR CONVERSATIONS field said that exact thing.
- If the client said they will NOT replace their TMS, do NOT pitch TMS replacement or ask timeline for a new TMS.
- Only state ${prospect}'s stack, pain points, and goals from: live transcript, PRIOR CONVERSATIONS, BACKGROUND, or CLIENT WEBSITE — nowhere else.`
}
